...
#include<iostream>
using namespace std;
int pool[200005],BTB[200005],st[200005];
...
void ID() 
{
    ...
    {
        fz=cur_id-4;
        if (BTB[fz]!=-1) bak_id=cur_id,yr=st[fz],cur_id=BTB[fz];
        else shut_if=1;
    }
    IR=0;
}
    ...
    if (opcode==99||opcode==111||opcode==103||opcode==23) 
    {
        ...
        else 
        {
            ...
            else if (pd&&st[fz]<2) 
            {
                ...
                shut_id=1;
                st[fz]= st[fz]==1?3:1;
                if (st[fz]==3) BTB[fz]=address;
            }
            else if (pd&&st[fz]>=2&&BTB[fz]!=address) 
            {
                cur_id=address;
                shut_id=1;
                BTB[fz]=address;
            }
            if (pd&&st[fz]==2) st[fz]=3;
            if (!pd&&st[fz]==1) st[fz]=0;
            yr=-1;
        }
    }
    pack(4);
